	...
	<div class="container-xl">
		...
		<div class="service-command-center">
			<div>
				...
				<p>Permintaan dari katalog publik masuk ke sini untuk disetujui, ditolak, atau ditandai selesai oleh petugas.</p>
			</div>
							...
							<tr>
								...
								<td data-label="Aksi">
									<?= form_open('catalog/requests/update/' . (int) $request['id'], ['class' => 'queue-action-form']); ?>
										<select class="form-select form-select-sm" name="status" aria-label="Ubah status request">
											<?php foreach (['approved', 'fulfilled', 'rejected', 'cancelled'] as $value): ?>
												<option value="<?= $value; ?>" <?= $request['status'] === $value ? 'selected' : ''; ?>><?= html_escape($status_labels[$value]); ?></option>
											<?php endforeach; ?>
										</select>
